...
```
Opus 4.8:high | c:12K/200K s:35%-◕ w:18%-d3 | main
...
- **Rate limits** (`s:` / `w:`) — 5-hour session and 7-day weekly usage percentages. The session value always carries a pie glyph whose fill is the fraction of the 5-hour window still remaining (`● ◕ ◑ ◔ ○`, full → empty as reset nears), e.g. `s:35%-◕`. Reading the pie against the percentage shows whether you're burning fast — high `%` with a still-full pie means you're spending quota early in the window. Once session usage reaches 90%, the percentage turns yellow and the exact countdown is appended (e.g. `s:93%-◔40m`). The weekly value carries a `-d<N>` suffix for which day (1–7) of the current 7-day window you're on (e.g. `w:18%-d3`), and its percentage likewise turns yellow at 90%.
